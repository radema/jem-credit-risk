...
def main():
    ...
    model = TabularJEM(input_dim=input_dim, num_classes=num_classes, config=config).to(device)
    ...
    df_train = pl.DataFrame({
        ...
        **{f"feat_{i}": np.random.randn(batch_size * num_batches) for i in range(input_dim)}
    })
    df_val = df_train.clone()

    train_loader, val_loader, scaler, feature_cols = get_dataloaders(df_train, df_val, batch_size=batch_size)
