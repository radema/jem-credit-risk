...
def extract_relevant_parquets(zip_path: str, cache_dir: str = ".cache") -> str:
    """
    Extracts strictly the 'train_' parquet files from the Kaggle dataset zip.
    ...
    logger.info(f"Extracting relevant 'train_' parquets from {zip_path} to {cache_dir}")
    extracted_count = 0
    with zipfile.ZipFile(zip_path, "r") as archive:
        all_files = archive.namelist()
        # Filter for train parquet files (ignoring test/sample submission and csvs)
        # Note: sometimes files are under a subdirectory in the zip, so we check the basename
        train_parquets = [
            f
            for f in all_files
            if f.endswith(".parquet") and "train_" in os.path.basename(f)
        ]
